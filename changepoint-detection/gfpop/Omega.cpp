...
void Omega::gfpopTest(Data const& data){
	Point* myData = data.getVecPt();
	n = data.getn();
	initialize_LP_ts_TEST(myData[0], n);
}

void Omega::initialize_LP_ts_TEST(Point firstData, unsigned int n)
{
	Interval inter = cost_interval(); ///get the cost-dependent interval
}